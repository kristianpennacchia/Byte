    ...
    var body: some View {
        ZStack {
            Circle()
                ...
                .foregroundColor(heartbeatChanged ? .brand.twitch.opacity(0.8) : .brand.primary)
                .animation(.spring(response: 0.3, dampingFraction: 0.3, blendDuration: 0.3), value: heartbeatChanged)
            Image("LogoShort")
                .foregroundColor(heartbeatChanged ? .brand.primary : .brand.twitch)
                .font(.system(size: 100))
                .scaleEffect(heartbeatChanged ? 1.5 : 1.0)
                .animation(.spring(response: 0.3, dampingFraction: 0.3, blendDuration: 0.3), value: heartbeatChanged)
        }
        .animation(.default, value: heartbeatChanged)
        .onFirstAppear {
